...
//   Form states 
  const [value1, setValue1] = useState("");
    ...
    const handleNumberOfCopies = (e) => {
        setNumberOfCopiesState(e.target.value)
    }
...
sx={{width: "30rem", display: "flex", padding: "30px 30px", borderRadius: 10 }}> */}
      ...
      <Grid container spacing={0} style={{
        ...
        // m: 5, 
        mb: 10 }}>
          <PageHeader2>Request - Cheque Book Request</PageHeader2>
          <SubTitle2 sx={{fontSize: {xs: 8, md: 10, lg: 12, marginTop: 5 }}}>
          Order a new cheque book with ease - Cheque Book Request 
          </SubTitle2>
...
<Grid item xs={3}sx={{mt: 1.5}} >
        ...
        <NameLabel sx={{}}>Number Of Copies  <span style={{color: "red"}}>*</span></NameLabel>
</Grid>
...
<Grid item xs={9} sx={{display: "flex"}} >
    ...
    <TextField
      ...
      // label="Branch"
      placeholder="-----------Select-------------"
      value={numberOfCopiesState}
      onChange={handleNumberOfCopies}
    >
      {numberOfCopies.map((option) => (
        <MenuItem key={option.value} value={option.value}>
          {option.label}
        </MenuItem>
      ))}
